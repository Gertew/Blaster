...
void ULagCompensationComponent::ServerScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
	...
	{
		const float Damage = Confirm.bCritical ? Character->GetEquippedWeapon()->GetHeadshotDamage() : Character->GetEquippedWeapon()->GetDamage();
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	// Enable Body Collision Disable Head Collision
	for (auto& Frame : FramePackages)
	{
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	// HitTime is too old Player is too laggy
	if (OldestHistoryTime > HitTime)
	{
		return FFramePackage();
	}
	// HitTime is the oldest in the history
	if (OldestHistoryTime == HitTime)
	{
		FrameToCheck = HitCharacterHistory.GetTail()->GetValue();
		bShouldInterpolate = false;
	}
	// HitTime is the youngest in the history
	...
	// While Older is younger than HitTime
	while (Older->GetValue().Time > HitTime)
	{
		// Go backwards until OlderTime < HitTime < YoungerTime
